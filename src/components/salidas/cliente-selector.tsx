...
export function ClienteSelector({
  ...
  const [lista, setLista] = useState<ClienteLite[]>(clientes);
  ...
  return (
    <Modal
      title={modo === "crear" ? "Nuevo cliente" : "Seleccionar cliente"}
      description={
        modo === "crear"
          ? "Consulta por RUC/DNI para traer los datos oficiales."
          : "Busca y elige el cliente de la venta."
      }
            ...
            <div className="max-h-72 space-y-1 overflow-y-auto">
              {filtrados.length === 0 ? (
                <p className="py-8 text-center text-sm text-muted-foreground">
                  Sin clientes. Usa “Crear” para registrar uno.
                </p>
              ) : (
                filtrados.map((c) => {
