...
import type { BusquedaEntidad } from "@/lib/documento-entidad";

/**
 * Formulario genérico de alta por documento (RUC/DNI) con consulta Decolecta.
 * Reutilizable para clientes y proveedores: recibe las acciones por props.
 */
export function EntidadDocumentoForm({
  acciones,
  onCreated,
  redirigirA = "/",
  verHref,
  ...
  onCreated?: (e: EntidadLite) => void;
  redirigirA?: string;
  verHref?: (id: string) => string;
  noun?: string;
}) {
  ...
  const [resultado, setResultado] = useState<BusquedaEntidad | null>(null);
  ...
  const buscar = async () => {
    ...
    setBuscando(true);
    try {
      setResultado(await acciones.buscar(tipo, numero));
    } finally {
      setBuscando(false);
    }
  };
  ...
  const guardar = async () => {
    setGuardando(true);
    try {
      const r = await acciones.crear({ tipoDocumento: tipo, numeroDocumento: numero, nombreManual });
      if (!r.ok || !r.entidad) {
        toast.error(r.error ?? "No se pudo guardar.");
        return;
      }
      toast.success("Guardado");
      if (onCreated) onCreated(r.entidad);
      else {
  ...
  const registrado = resultado?.estado === "registrado" ? resultado.entidad : null;
  ...
  return (
    <div className="space-y-4">
      ...
      {registrado && (
        <div className="space-y-2 rounded-lg border border-[color:var(--warning)]/40 bg-[color:var(--warning)]/5 p-4">
          <p className="flex items-center gap-2 text-sm font-medium text-[color:var(--warning)]">
            <AlertCircle className="size-4" /> Este {tipo} ya está registrado como {noun}
          </p>
          ...
          {onCreated ? (
            <Button type="button" onClick={() => onCreated(registrado)}>
              <UserCheck className="size-4" /> Usar este {noun}
            </Button>
          ) : verHref ? (
            <Link href={verHref(registrado.id)} className={buttonVariants({ variant: "outline", size: "sm" })}>
              Ver {noun}
            </Link>
          ) : null}
        </div>
      ...
      <Button type="button" onClick={guardar} disabled={!puedeGuardar || guardando}>
        ...
        {guardando ? "Guardando…" : `Guardar ${noun}`}
      </Button>
    </div>
  );
}
